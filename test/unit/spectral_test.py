...
def test_equivalent():
    test_data = xarray.open_dataarray(datasets.TEST_HSI)
    test_data_x = xarray.open_dataarray(datasets.TEST_HSI, chunks=2)
    all_classes = _get_public_classes(spectral)
    assert len(all_classes) > 0
    for spc in all_classes:
        model = spc[1]()
        out = test_data.pipe(model.transform)
        outx = test_data_x.pipe(model.transform).compute()
        assert out.sum() > 1
        assert outx.sum() > 1
